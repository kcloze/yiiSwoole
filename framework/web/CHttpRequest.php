...
class CHttpRequest extends CApplicationComponent
{
	 ...
	 * @see getPost
	 */
	public function getParam($name,$defaultValue=null,$checkempty=false)
	{
		$param = isset($_GET[$name]) ? $_GET[$name] : (isset($_POST[$name]) ? $_POST[$name] : $defaultValue);
                if($checkempty) $param = !empty($_REQUEST[$name])?$_REQUEST[$name]:$defaultValue;//update 2014-6-17 qichao 增加是否为空判断
		return $param;
	}
